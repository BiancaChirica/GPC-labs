...
void Display1() {
    ...
    glBegin(GL_LINES); // trasarea unei linii , desenam o figura geometrica adica linii
    glVertex2i(1, 1); // 2 parametrii, imagine 2D, coordonatele unui varf , i - integer, float, double
    glVertex2i(-1, -1);
    glEnd();
...
void Display3() {
    //punem coordonatele exacte la puncte
...
void Display5() {
    // inchide ultimul cu primul , numarul de varfuri minim trebuie folosit
    glColor3f(1, 0.1, 0.1); // rosu
...
void Display7() {
    ...
    glPolygonMode(GL_FRONT, GL_FILL); // to be full inside
    glBegin(GL_QUADS);
    // ordinea varfurilor conteaza,  counter clock-wise
    glVertex2f(1.0, 1.0);//0
    glVertex2f(0.2, 0.7);//1
    glVertex2f(0.5, 0.5);//2
    glVertex2f(1.0, 0.5);//3

    glEnd();
}

void Display8() {
    // mai multe blocuri glbegin, dar nu mai mult de 3 !
    // zona rosie separator perfect intre zona rosie si cea albastra
    ...
    glPolygonMode(GL_FRONT, GL_LINE); // doar conturul
    glColor3f(1, 0.1, 0.1); // rosu
...
void KeyboardFunc(unsigned char key, int x, int y) {
    printf("Ati tastat <%c>. Mouse-ul este in pozitia %d, %d.\n",
        key, x, y);
    //tastele sunt date cu codul ascii
